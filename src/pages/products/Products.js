...
function Products() {
  ...
  const renderListCard = (product) => {
    ...
    return (
      <div key={product.id} className="bg-white rounded-3xl elegant-shadow p-5 flex flex-col md:flex-row items-center gap-6 border border-gray-50/50 hover:border-[#0f2a4a]/20 transition-all duration-300 select-none text-left">
        <div className="relative w-full md:w-48 h-48 rounded-2xl overflow-hidden cursor-pointer flex-shrink-0" onClick={() => navigate(`/product/${product.id}`)}>
          <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
          {hasDiscount && (
            <span className="absolute top-3 left-3 bg-[#d4af37] text-[#0b1a30] font-black text-[9px] px-2 py-0.5 rounded-full shadow">
        ...
        <div className="flex-1 flex flex-col justify-between py-1 space-y-4">
          <div>
            <span className="text-[10px] font-bold text-[#d4af37] tracking-wider uppercase">{product.category?.replace('-', ' ')}</span>
            <h3 className="text-lg font-bold text-gray-800 font-serif leading-tight mt-1 hover:text-[#0f2a4a] cursor-pointer" onClick={() => navigate(`/product/${product.id}`)}>{product.name}</h3>
          ...
          <div className="flex items-center justify-between border-t border-gray-50 pt-4">
            <div className="flex items-baseline gap-2">
              {hasDiscount ? (
                <>
                  <span className="text-xl font-extrabold text-[#0f2a4a]">₹{Number(product.discountedPrice).toFixed(0)}</span>
                  <span className="text-gray-400 line-through text-xs">₹{Number(product.price).toFixed(0)}</span>
                </>
              ) : (
                <span className="text-xl font-extrabold text-[#0f2a4a]">₹{Number(product.price).toFixed(0)}</span>
              )}
            </div>
            <button 
              onClick={() => navigate(`/product/${product.id}`)}
              className="bg-[#0f2a4a] hover:bg-[#1b4965] text-white px-5 py-2 rounded-xl text-xs font-bold transition-all"
            >
              View Details
            </button>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16">
      ...
      <nav className="text-[10px] text-gray-500 mb-4 flex items-center gap-1.5 select-none font-bold uppercase tracking-wider">
        ...
        <span>/</span>
        <Link to="/products" className="hover:text-[#0f2a4a] transition-colors">Products</Link>
      ...
      {didYouMean && (
        <div className="mb-6 text-xs font-bold text-gray-600 bg-blue-50 border border-blue-100 rounded-xl px-4 py-2.5 text-left select-none">
          🔍 Did you mean: <button onClick={() => setSearchQuery(didYouMean)} className="text-[#0f2a4a] hover:underline font-extrabold">{didYouMean}</button>?
        </div>
      )}
          ...
          <div className="bg-white rounded-3xl elegant-shadow p-5 md:p-6 sticky top-28 border border-gray-100 space-y-6">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-[#0f2a4a] bg-white text-gray-700"
              >
          ...
          <div className="flex justify-between items-center mb-6 bg-white px-5 py-4 rounded-2xl elegant-shadow border border-gray-50/50">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className={`p-2 rounded-lg border transition-all ${viewMode === 'grid' ? 'bg-[#0f2a4a] border-[#0f2a4a] text-white' : 'bg-gray-50 border-gray-200 text-gray-400 hover:text-gray-600'}`}
                title="Grid View"
              >
              ...
              <button 
                ...
                className={`p-2 rounded-lg border transition-all ${viewMode === 'list' ? 'bg-[#0f2a4a] border-[#0f2a4a] text-white' : 'bg-gray-50 border-gray-200 text-gray-400 hover:text-gray-600'}`}
                title="List View"
              >
              ...
              <div className="mt-6 flex flex-wrap justify-center gap-2 max-w-md mx-auto">
                {['Choker', 'Silver Plate', 'Panchaloha Ring', 'Kasu Mala', 'German Silver'].map(term => (
                  <button 
                    ...
                    className="bg-[#f0f5fa] border border-[#0f2a4a]/10 hover:border-[#0f2a4a] px-3.5 py-2 rounded-xl text-[10px] font-bold text-[#0f2a4a] transition-all"
                  >
                    "{term}"
                  </button>
                ))}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 mt-12 pt-6 border-t border-gray-100 select-none">
              <button
                ...
                className="px-4 py-2 border border-gray-200 rounded-xl text-xs font-bold bg-white text-[#0f2a4a] hover:bg-gray-50 transition-all disabled:opacity-50"
              >
                ◀ Prev
              </button>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(num => (
                <button
                  ...
                  className={`w-9 h-9 flex items-center justify-center rounded-xl text-xs font-bold border transition-all ${currentPage === num ? 'bg-[#0f2a4a] border-[#0f2a4a] text-white shadow-md' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                  {num}
                </button>
              ))}
