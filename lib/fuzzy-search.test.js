...
describe('searchProducts', () => {
  ...
  it('does not fuzzy-match everything just because descriptions share common short words', () => {
    // Regression test: "a"/"an"/"for" etc. appear in nearly every description and are trivially
    // "contained in" almost any longer query — they must never cause a spurious match.
    const unrelated = [
      { id: '5', name: 'Panchamukhi Hanuman Idol', description: 'A striking idol featuring a vibrant finish for festive display', category: 'gifts' },
    ];
    const results = searchProducts(unrelated, 'neklace');
