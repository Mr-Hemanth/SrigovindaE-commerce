...
describe('calculateShippingCost', () => {
  it('charges the standard fee below the free-shipping threshold', () => {
    expect(calculateShippingCost(FREE_SHIPPING_THRESHOLD - 1, 'standard')).toBe(STANDARD_SHIPPING_FEE);
  });

  it('is free at exactly the free-shipping threshold', () => {
    expect(calculateShippingCost(FREE_SHIPPING_THRESHOLD, 'standard')).toBe(0);
  });

  it('is free above the free-shipping threshold', () => {
    expect(calculateShippingCost(FREE_SHIPPING_THRESHOLD + 500, 'standard')).toBe(0);
  });

  it('always charges the express fee regardless of subtotal', () => {
    expect(calculateShippingCost(0, 'express')).toBe(EXPRESS_SHIPPING_FEE);
    expect(calculateShippingCost(FREE_SHIPPING_THRESHOLD + 500, 'express')).toBe(EXPRESS_SHIPPING_FEE);
  });
});
