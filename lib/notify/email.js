...
const OWNER_EMAIL = 'info@srigovindacollections.com';
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px; width:100%; background-color:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 1px 4px rgba(0,0,0,0.06);">
            ...
            <tr>
              <td style="background-color:${NAVY}; padding:24px 32px; text-align:center;">
                ...
                <p style="margin:0 0 10px; color:#bbbbbb; font-size:11px;">+91 9533866777 &nbsp;·&nbsp; support@srigovindacollections.com</p>
    ...
    .map((item) => {
      ...
      const imgCell = item.image
        ? `<td width="64" style="padding:12px 12px 12px 0; vertical-align:top;">
             <img src="${escapeHtml(optimizeCloudinaryUrl(item.image, { width: 112 }))}" width="56" height="56" alt="" style="display:block; width:56px; height:56px; object-fit:cover; border-radius:8px; border:1px solid ${BORDER};" />
           </td>`
        : '';
      return `
        <tr>
...
function reviewItemRowsHtml(items, siteUrl) {
  return (items || [])
    .map((item) => {
      const imgCell = item.image
        ? `<td width="64" style="padding:12px 12px 12px 0; vertical-align:top;">
      ...
      return `
        <tr>
          ${imgCell}
          <td style="padding:12px 0; vertical-align:middle; border-bottom:1px solid ${BORDER}; font-size:13px; color:#333;">
            <div style="font-weight:bold; color:${NAVY};">${escapeHtml(item.name)}</div>
          </td>
          <td style="padding:12px 0; vertical-align:middle; border-bottom:1px solid ${BORDER}; text-align:right; white-space:nowrap;">
            <a href="${siteUrl}/product/${encodeURIComponent(item.id)}" style="display:inline-block; padding:8px 16px; color:${NAVY}; font-size:12px; font-weight:bold; text-decoration:none; border:1px solid ${NAVY}; border-radius:8px;">Write a Review</a>
          </td>
        </tr>`;
    })
    .join('');
...
export async function sendReviewNudgeEmail(order, siteUrl) {
  const body = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td align="center" style="padding-bottom:8px;">
          <h1 style="margin:0; font-family:Georgia,'Times New Roman',serif; font-size:20px; color:${NAVY};">How's your new piece, ${escapeHtml(order.userName) || 'there'}?</h1>
        </td>
      </tr>
      <tr>
        <td align="center" style="padding-bottom:20px;">
          <p style="margin:0; font-size:13px; color:#777;">Your order #${escapeHtml(order.orderId)} was delivered a few days ago. A quick review helps other shoppers — and takes less than a minute.</p>
        </td>
      </tr>
    </table>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      ${reviewItemRowsHtml(order.items, siteUrl)}
    </table>
  `;

  return sendEmail({
    to: order.userEmail,
    subject: `${order.userName || 'Hi'}, how was your Sri Govinda Collections order?`,
    html: renderShell({ preheader: 'Share a quick review of your recent purchase.', bodyHtml: body, footerLine: 'Thank you for shopping with us' }),
  });
}

// Sent to the store owner the moment a coupon is created — manually in the admin panel, or by
// the Mon/Wed/Fri auto-generation cron (see app/api/cron/daily-coupon/route.js) — so the code is
// in their inbox instantly instead of requiring a trip to the admin panel to find it.
export async function sendCouponCreatedEmail(coupon) {
  const body = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td align="center" style="padding-bottom:16px;">
          <h1 style="margin:0; font-family:Georgia,'Times New Roman',serif; font-size:22px; color:${NAVY};">New Coupon Created</h1>
        </td>
      </tr>
      <tr>
        <td style="background-color:${CREAM}; border-radius:10px; padding:18px 20px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="font-size:11px; color:#999; text-transform:uppercase; letter-spacing:0.5px;">Code</td>
              <td style="text-align:right; font-size:11px; color:#999; text-transform:uppercase; letter-spacing:0.5px;">Discount</td>
            </tr>
            <tr>
              <td style="font-size:20px; font-weight:bold; color:${NAVY}; font-family:monospace;">${escapeHtml(coupon.code)}</td>
              <td style="text-align:right; font-size:18px; font-weight:bold; color:${NAVY};">${escapeHtml(String(coupon.discountPercentage))}% OFF</td>
            </tr>
          </table>
        </td>
      </tr>
      <tr>
        <td style="padding-top:16px;">
          <p style="margin:0; font-size:13px; color:#555; line-height:1.6;">
            Expires: <strong style="color:${NAVY};">${formatDate(coupon.expiryDate)}</strong><br/>
            Uses per customer: <strong style="color:${NAVY};">${coupon.maxUsesPerUser ? escapeHtml(String(coupon.maxUsesPerUser)) : 'Unlimited'}</strong>
          </p>
        </td>
      </tr>
    </table>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td align="center">
          ${ctaButton(`${SITE_URL}/admin/coupons`, 'View in Admin Panel')}
        </td>
      </tr>
    </table>
  `;
  ...
  return sendEmail({
    to: OWNER_EMAIL,
    subject: `New Coupon Created — ${coupon.code} (${coupon.discountPercentage}% OFF)`,
    html: renderShell({
      preheader: `Coupon ${coupon.code} for ${coupon.discountPercentage}% off is now live.`,
      bodyHtml: body,
      footerLine: 'Admin coupon notification',
    }),
  });
}
