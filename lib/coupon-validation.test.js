import { isCouponExpired, isCouponValid, isCouponUsageLimitReached } from '@/lib/coupon-validation';
...
describe('isCouponUsageLimitReached', () => {
  it('is never reached when maxUsesPerUser is unset', () => {
    expect(isCouponUsageLimitReached(undefined, 999)).toBe(false);
  });

  it('is never reached when maxUsesPerUser is 0', () => {
    expect(isCouponUsageLimitReached(0, 999)).toBe(false);
  });

  it('is not reached when usageCount is below the limit', () => {
    expect(isCouponUsageLimitReached(2, 1)).toBe(false);
  });

  it('is reached when usageCount equals the limit', () => {
    expect(isCouponUsageLimitReached(2, 2)).toBe(true);
  });

  it('is reached when usageCount exceeds the limit', () => {
    expect(isCouponUsageLimitReached(1, 5)).toBe(true);
  });
});
